...
- Description: Testcontainers (MongoDB + Postgres for notification, Postgres for ticket, Kafka) tests
  covering event-to-notification dispatch with preference suppression and idempotency, template
  rendering, ticket open/assign/resolve with events, and SLA breach. Verify AC-01 welcome SMS, AC-02 invoice email, and AC-03 threshold SMS now dispatch.
