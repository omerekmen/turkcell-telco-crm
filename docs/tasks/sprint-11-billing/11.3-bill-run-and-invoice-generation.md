...
- Description: Render each invoice to PDF and store it in MinIO (local FS only as a dev fallback),
  recording `pdf_ref` (object reference) on the invoice; the PDF is served via a time-limited
  pre-signed URL from the read API and referenced in notifications (ADR-006, FR-23).
