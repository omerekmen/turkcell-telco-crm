...
class RecordSubscriptionTerminatedCommandHandlerTest {

    @Mock private SubscriberBillingRecordRepository subscriberRepo;

    private RecordSubscriptionTerminatedCommandHandler handler;

    @BeforeEach
    void setUp() {
        handler = new RecordSubscriptionTerminatedCommandHandler(subscriberRepo);
    }

    @Test
    void terminates_the_matching_billing_record() {
        ...
        handler.handle(new RecordSubscriptionTerminatedCommand(subscriptionId, terminatedAt));
        ...
        assertThat(captor.getValue().getStatus()).isEqualTo(SubscriberBillingRecord.TERMINATED);
        assertThat(captor.getValue().getTerminatedAt()).isEqualTo(terminatedAt);
    }
    ...
    void is_a_no_op_when_no_billing_record_exists_for_the_subscription() {
        ...
        handler.handle(new RecordSubscriptionTerminatedCommand(subscriptionId, Instant.now()));
