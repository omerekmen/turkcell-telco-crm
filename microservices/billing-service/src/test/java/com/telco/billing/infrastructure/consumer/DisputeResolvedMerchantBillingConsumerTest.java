...
class DisputeResolvedMerchantBillingConsumerTest {

    @Mock private Mediator mediator;
    @Mock private InboxService inboxService;

    private DisputeResolvedMerchantBillingConsumer consumer;
    private final ObjectMapper objectMapper = new ObjectMapper();

    @BeforeEach
    void setUp() {
        consumer = new DisputeResolvedMerchantBillingConsumer(mediator, inboxService, objectMapper);
    }
    ...
    @Test
    void clears_hold_on_referenced_invoice() {
        UUID invoiceId = UUID.randomUUID();
        when(inboxService.firstSeen(anyString(), eq("DisputeResolvedMerchantBillingConsumer"))).thenReturn(true);
        ...
        verify(mediator).send(new ClearInvoiceDisputeHoldCommand(invoiceId));
    }

    @Test
    void no_ops_when_invoiceId_is_null() {
        ConsumerRecord<String, String> record = record(
