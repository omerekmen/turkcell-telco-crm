package com.telco.usage.migration;
...
@Testcontainers
class UsageSchemaMigrationTest {
    ...
    void migrations_apply_cleanly_and_create_expected_schema() throws Exception {
        ...
        try (Connection c = DriverManager.getConnection(
            ...
            for (String table : new String[]{"quotas", "usage_records", "outbox_event"}) {
                assertTrue(tableExists(meta, table), "table missing: " + table);
            }

            assertTrue(importedTables(meta, "usage_records").contains("quotas"),
                    "usage_records missing FK to quotas");
        }
    }
