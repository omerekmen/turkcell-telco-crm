package com.telco.identity.migration;
...
/**
 * Verifies the identity Flyway migrations apply cleanly on a real PostgreSQL container and produce the
 * expected RBAC projection and audit schema (feature 5.1.2, ADR-013, ADR-016).
 *
...
class IdentitySchemaMigrationTest {
    ...
    void migrationsApplyCleanlyAndCreateSchema() throws Exception {
        ...
        try (Connection connection = DriverManager.getConnection(
            ...
            assertTrue(importedTables(metaData, "user_roles").containsAll(Set.of("users", "roles")),
                    "user_roles is missing foreign keys to users/roles");
            assertTrue(importedTables(metaData, "role_permissions").containsAll(Set.of("roles", "permissions")),
                    "role_permissions is missing foreign keys to roles/permissions");
        }
    }
