...
/**
 * Confirms a PENDING order after payment completes (saga step {@code PAYMENT_COMPLETED}).
 * Dispatched by the {@code payment.completed.v1} consumer. Drives {@code Order.confirm()}
 * (PENDING -&gt; CONFIRMED) and advances saga_state. No order domain event is emitted: the transition
 * is a local state change (AC-01).
 *
...
public record ConfirmOrderCommand(
        ...
        /** payment id for saga payload / traceability (may be null). */
        String paymentId,
